    ...
    try {
        ...
        if (data["PLANT"] !== "NOdata") {
            ...
            if (check.length === 0) {
                ...
                var ins = await mongodb.insertMany(`${neworder['PLANT']}dbMAIN`, 'MAIN', [neworder]);
                output = `The order have added to PLANT:${data["PLANT"]}`;
                query = `INSERT  INTO [SOI8LOG].[dbo].[qcbypass_weight] ([POID],[COMMENT],[STATUS],[USERID]) VALUES ('${input['PO']}','${input['COMMENT']}','NEW','${input['UID']})`
                ...
                if (check2.length === 0) {
                    ...
                    query = `INSERT  INTO [SOI8LOG].[dbo].[qcbypass_weight] ([POID],[COMMENT],[STATUS],[USERID]) VALUES ('${input['PO']}','${input['COMMENT']}','HAVE','${input['UID']})`
                    let db = await mssqlR.qureyR(query);
                    output = `The order have already had in DB`;
                }
